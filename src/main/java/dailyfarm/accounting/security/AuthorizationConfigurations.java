	...
	SecurityFilterChain configure(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(authorize -> authorize
				...
				.requestMatchers("/supplier/revoke/*", "/customer/revoke/*", "/supplier/activate/*",
						"/customer/activate/*")
				.hasRole("ADMIN").requestMatchers("/customer/{login}/role/{role}", "/supplier/{login}/role/{role}")
				.hasRole("ADMIN").requestMatchers(HttpMethod.PUT, "/supplier/{login}", "/customer/{login}")
